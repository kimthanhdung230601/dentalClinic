...
const CalendarCustom: React.FC<CalendarProps> = ({
  ...
  const value = (room: string, shift: string) => {
    if (room === "Thuận Bình" && shift === "2") {
      return "Nghỉ";
    } else if (room === "Lý Đào" && shift === "3") {
      return "Lịch khám";
    } else if (room === "Nguyễn Châu Anh" && shift === "4") {
      return "Lịch họp";
    } else {
      return ""; // Trả về chuỗi rỗng nếu không phù hợp với bất kỳ điều kiện nào
    }
  };
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [shift, setShift] = useState();
        ...
        <div className={styles.row} key={shift}>
          ...
          {dates.map((date) => (
            <div key={date} className={styles.day}>
              <div className={styles.roomList}>
                <Row>
                  {operator_name.map((room, index) => (
                    <Col
                      span={6}
                      key={index}
                      className={styles.record}
                      onClick={() => {
                        showModal( shift, date, room);
                      }}
                    >
                      {value(room, shift) === "Nghỉ"
                        ? "Xin nghỉ: Bị covid"
                        : value(room, shift) === "Lịch khám"
                        ? "Lịch khám: Nguyễn Anh- ND: Tái khám"
                        : value(room, shift) === "Lịch họp"
                        ? "Lịch họp: Nghiên cứu cấy ghép răng"
                        : ""}
                    </Col>
                  ))}{" "}
                </Row>
